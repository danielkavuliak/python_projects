...
class PolicyValueNet():
    def __init__(self, board, model_file=None):
        self.board = board

        print("dostal som sa do policy value")
        # Define the tensorflow neural network
        # 1. Input:
        self.input_states = tf.keras.backend.placeholder(
                tf.float32, shape=[None, 4, board])
        print("preskocil som")
        ...
        self.evaluation_conv_flat = tf.reshape(
                self.evaluation_conv, [-1, 2 * board])
        ...
        self.mcts_probs = tf.keras.backend.placeholder(
                tf.float32, shape=[None, board])
        self.policy_loss = tf.negative(tf.reduce_mean(
                tf.reduce_sum(tf.multiply(self.mcts_probs, self.action_fc), 1)))
        # 3-3. L2 penalty (regularization)
        l2_penalty_beta = 1e-4
        vars = tf.keras.backend.trainable_variables()
